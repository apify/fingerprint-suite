    ...
    return z.preprocess(
        ...
        z.object({
            ...
            browserFingerprint: z.object({
                ...
                audioCodecs: z.record(z.enum(["ogg", "mp3", "wav", "m4a", "aac"]), z.enum(["probably", "maybe", ""])),
                videoCodecs: z.record(z.enum(["ogg", "h264", "webm"]), z.enum(["probably", "maybe", ""])),
                ...
                userAgent: z.string()
                    .refine(
                        (userAgent) => !userAgent.match(/(bot|bots|slurp|spider|crawler|crawl)\b/i),
                        "User agent should not contain bot, bots, slurp, spider, crawler, or crawl"
                    )
                    .refine(
                        (userAgent) => !robotUserAgents.some((robot) => userAgent.match(new RegExp(robot.pattern, 'i'))),
